...
read 
#y=$(find -type f -name "$REPLY" < $dbpath | wc -l )
			...
			then
				...
				then
					...
					then 
						...
						echo "continue"		
						#going to update more than pk
				oldval=$(echo ${inputvalue[0]} | sed 's/,/ /g')
			morethanone=$(sed '1,3d' $dbpath/$tbname | awk -v pal="$inputfone" -v val="$oldval" -F"," '{if($pal==val)print 1;}'| wc -l)
					
					if [ $morethanone -gt 1 ]
					then
						pkcheck=1
						echo "you are to make more thank row with that pk"
